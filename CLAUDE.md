...
index.ts → parseInputs() → collectSourceTexts() → extractKeysFromTexts() → setOutput()
                                                                          ...
                                                                          → appendJiraLinksToPr() (if github_token set)
```
...
- **`pr-links.ts`** — Appends Jira ticket links to PR body via GitHub API. Uses HTML comment markers for idempotent updates
